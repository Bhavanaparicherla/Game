...
class App extends Component {
  ...
  timeDecrement = () => {
    ...
    if (timerRunning !== 0) {
      this.setState(prevState => ({timerRunning: prevState.timerRunning - 1}))
    } else {
  ...
  getFilteredImgs = () => {
    ...
    const filterImgList = imagesList.filter(
      each => each.category === activeTabiD,
    )
    return filterImgList
  }

  selectedThumbnail = thumbnailUrl => {
    const {timerRunning, score, initialImg} = this.state

    const imgurlValue = imagesList.filter(
      each => each.thumbnailUrl === thumbnailUrl,
    )
    const {imageUrl} = imgurlValue[0]
    const findingImgList = imageUrl === initialImg
    console.log(findingImgList) //true

    if (findingImgList) {
      const indexImg = Math.floor(Math.random() * 30)
      this.setState(prevState => ({
        initialImg: imagesList[indexImg].imageUrl,
        score: prevState.score + 1,
      }))
    } else {
      clearInterval(this.timerId)
      this.setState(prevState => ({gameOn: false}))
    }
  }
  ...
  render() {
    const {timerRunning, score, activeTabiD, initialImg, gameOn} = this.state
    const filteredImages = this.getFilteredImgs()
    ...
    return (
      <div className="bg-container">
        <nav className="nav-bar-container">
          ...
          <div className="time-runner-container">
            <p>Score: {score}</p>
            <img
        ...
        <div className="tab-img-container">
          {gameOn ? (
            <div className="img-section-container">
                  ...
                  {tabsList.map(each => (
                    <li key={each.tabId}>
                      <button
                        type="button"
                        className="btn"
                        onClick={() => this.selectTab(each.tabId)}
                      >
                        {each.displayText}
                      </button>
                    </li>
              ...
              <div>
                <ul className="ul-image-container">
                  {filteredImages.map(eachImagelist => (
                    <li className="image-container" key={eachImagelist.id}>
                      <button
                        type="button"
                        onClick={() =>
                          this.selectedThumbnail(eachImagelist.thumbnailUrl)
                        }
                        className="button-style"
                      >
                        <img
                          src={eachImagelist.thumbnailUrl}
                          alt="thumbnail"
            ...
            <div className="win-container">{this.renderScorestatus()}</div>
          )}
        </div>
      </div>
    )
